...
if(isset($_POST['payNow'])){
    if($_POST['payment_method'] == 'PAYSAFECARD'){
        if($_POST['amount'] >= $min_payment && $_POST['amount'] <= $max_payment && is_numeric($_POST['amount'])){
            ...
            $SQL = $odb->prepare("INSERT INTO `transactions`(`user_id`, `gateway`, `state`, `amount`, `desc`, `tid`) VALUES (:user_id,:gateway,'PENDING',:amount,'Guthabenaufladung mit paysafecard',:tid)");
            $SQL->execute(array(":user_id" => $_SESSION['id'], ":gateway" => 'paysafecard', ":amount" => $_POST['amount'], ":tid" => $tid));
...
$SQLSelectServers = $odb -> prepare("SELECT * FROM `transactions` WHERE `user_id` = :user_id AND `state` = 'PENDING' AND `gateway` = 'paysafecard'");
