...
function getViewer($ts3_query, $picUrl, $serverStatus){
    if($serverStatus == 'ONLINE') {
		try{
			...
			echo 'Es konnten nicht alle Server-Icons geladen werden.';
		}
    } else {
        echo 'Der Teamspeak muss gestartet sein.';
    }
}
...
function modify($ts3_query, $slots, $port){
    try {
        return $ts3_query->modify(
            [
